...
export default function PostItemLikes({ className: cls, postId }: Props) {
  const { onHoverEnd, onHoverStart } = useTooltip();

  const { data, loading } = useGetPostReactionsCountQuery({
    fetchPolicy: "network-only",
    notifyOnNetworkStatusChange: true,
    variables: { id: postId },
  });

  const count = data?.postReactionsCount.count ?? 0;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-center">
        <span className="relative flex size-5 cursor-pointer select-none items-center justify-center fill-secondary text-xl">
      ...
      <span className={cn(className.textBtn, "ml-1")}>
        {countConvert(count)}
      </span>
    </span>
  );
