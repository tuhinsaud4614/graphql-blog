...
import { cn, generateFileUrl, maxFileSize } from "@/lib/utils";
...
export function Upload({ onAdd }: Readonly<Props>) {
  ...
  const onChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files?.length) {
      ...
      try {
        ...
        if (file && !_.has(IMAGE_MIMES, file.type)) {
          setError("File should be image (gif, svg, jpeg, jpg, png, webp)");
        } else if (file && file.size > maxFileSize(2)) {
