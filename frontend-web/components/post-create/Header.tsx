...
export default function Header({ hideAvatar = false }: Readonly<Props>) {
  return (
    <header className={className.root}>
      <nav className={className.nav}>
        <Link href={ROUTES.user.home} className={className.homeLink} aria-label="Home">
            <LogoIcon className="size-[3.125rem]" />
        </Link>
        <ClientOnly>
          {!hideAvatar && (
            <UserAvatarBtn
