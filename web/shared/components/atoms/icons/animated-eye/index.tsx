...
interface AnimatedEyeIconProps extends SVGMotionProps<SVGSVGElement> {
  isVisible?: boolean;
  size?: number;
  variant?: IconVariant;
}

const AnimatedEyeIcon = React.forwardRef<SVGSVGElement, AnimatedEyeIconProps>(
  ({ isVisible = true, size = 24, variant = "normal", ...props }, ref) => {
    ...
    return (
      <motion.svg
        ...
        {...props}
      >
        {/* Eye Outline - Always visible but could morph if we had exact paths for it. 
            For now, we'll keep the open eye outline and overlay the closed slash/arc. 
            Actually, let's toggle between the two main shapes for a cleaner effect.
        */}
        ...
        >
          <path
            ...
            strokeLinecap="round"
            strokeWidth={35.938}
            d="M468.75 203.125c-46.875 46.875-125 62.5-218.75 62.5S78.125 250 31.25 203.125M109.375 250l-62.5 62.5M390.625 250l62.5 62.5"
          />
          {/* The slash needs to be animated separately if we want a drawing effect, 
              but for now a crossfade is safer with these complex paths. 
              Let's add the vertical slash.
          */}
          <motion.path
            className={stroke}
            stroke="currentColor"
