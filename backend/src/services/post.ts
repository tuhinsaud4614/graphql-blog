...
export async function createUntitledPostService(
  prisma: PrismaClient,
  authorId: string,
) {
  try {
    return await createUntitledPost(prisma, authorId);
  } catch (error) {
    logger.error(error);
...
export async function updatePostDraftService(
  prisma: PrismaClient,
  params: UpdatePostDraftInput,
  userId: string,
) {
  try {
    await updatePostDraftSchema.validate(params, { abortEarly: false });
  } catch (error) {
    logger.error(error);
    return formatError(error, { key: "post draft" });
  }

  try {
    const isExist = await getAuthorPostById(prisma, params.id, userId);

    if (!isExist) {
      return new NoContentError(generateNotExistErrorMessage("Post"));
    }

    await updatePostDraft(prisma, params);
    return "Post draft updated successfully";
  } catch (error) {
    logger.error(error);
    return new UnknownError(generateUpdateErrorMessage("Post draft"));
  }
}
...
export async function publishPostService(
  ...
  try {
    ...
    logger.error(error);
    return new UnknownError("Failed to publish the Post");
  }
}
...
/**
 * This function retrieves a post by ID and returns it, or an error if the post
 * does not exist. If the post is a draft and the requesting user is not the
 * author, it returns the post with draft set to null.
 * @param {PrismaClient} prisma - The PrismaClient instance used to interact with the database.
 * @param {string} id - The ID of the post to retrieve.
 * @param {string} [authUserId] - The ID of the user who is requesting the post. If this is
 * provided and the post is a draft, the function will return the post with the draft
 * property set to null.
 * @returns The post with the specified ID, or an error if the post does not exist.
 */
export async function postByIdService(
  prisma: PrismaClient,
  id: string,
  authUserId?: string,
) {
  try {
    // Retrieve the post by ID. If it doesn't exist, an error will be thrown.
    const result = await getPostById(prisma, id, true);

    if (result && authUserId !== result.authorId) {
      result.draft = null;
    ...
    // If an error occurs, log it and return an UnknownError with a message.
    logger.error(error);
